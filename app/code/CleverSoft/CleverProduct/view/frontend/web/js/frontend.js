...
define(['jquery', 'jquery/ui',"js/jquery.noconflict",'domReady'], function($, ui,no,domReady) {
    ...
    $.widget('mage.cleverProductMage', {
        ...
        //initCarouselCallBack: function(){
        //    this.initCarouselAnimation(this.element, true);
        //    this.initBackground();
        //},
        initCountdown: function () {
            ...
            if (!$.fn.countdown) {
                $.getScript(seft.options.countdown.engineSrc, function () {
        ...
        bindAnimateEvent: function (visibleItems) {
            var seft = this;
            'DOMContentLoaded|load|resize|scroll'.split('|').each(function (event,val) {
                ...
                $(tab).on('click', function(e){
                    ...
                    $.ajax({
                        ...
                        success: function(transport){
                            ...
                            if(imag.length > 0) {
                                imag.each(function(){
                                    $(this).closest('a').addClass('_loading show');
                                });
                                imag.lazyload({
                                    data_attribute: "src",
                                    failure_limit : 10
        ...
        initCarousel: function (callback) {
            ...
            if (!$.fn.owlCarousel) {
                $.getScript(seft.options.carousel.engineSrc, function () {
        ...
        initCarouselElement: function (config) {
            ...
            this.container.find('.owl-carousel').each(function () {
                ...
                if(config.lazyload) {
                    if (!$.fn.lazyload) {
                        $.getScript(config.lazyEngineSrc).done(function( script, textStatus ) {
        ...
        bindParallaxEvent: function (elm) {
            var seft = this;
            'DOMContentLoaded|load|resize|scroll|video:load'.split('|').each(function (i,event) {
